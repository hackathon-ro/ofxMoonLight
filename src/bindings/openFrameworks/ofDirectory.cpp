...
static int ofDirectory_ofDirectory(lua_State *L) {
  try {
    ...
    if (top__ >= 1) {
      ...
      ofDirectory *retval__ = new ofDirectory(std::string(path, path_sz_));
...
static int ofDirectory_open(lua_State *L) {
  try {
    ...
    self->open(std::string(path, path_sz_));
    return 0;
  } catch (std::exception &e) {
...
static int ofDirectory_allowExt(lua_State *L) {
  try {
    ofDirectory *self = *((ofDirectory **)dub_checksdata(L, 1, "ofDirectory"));
    size_t extension_sz_;
    const char *extension = dub_checklstring(L, 2, &extension_sz_);
    self->allowExt(std::string(extension, extension_sz_));
    return 0;
  } catch (std::exception &e) {
...
static int ofDirectory_listDir(lua_State *L) {
  try {
    ...
    if (top__ >= 2) {
      size_t path_sz_;
      const char *path = dub_checklstring(L, 2, &path_sz_);
      lua_pushnumber(L, self->listDir(std::string(path, path_sz_)));
      return 1;
    } else {
...
static int ofDirectory_getOriginalDirectory(lua_State *L) {
  try {
    ofDirectory *self = *((ofDirectory **)dub_checksdata(L, 1, "ofDirectory"));
    lua_pushlstring(L, self->getOriginalDirectory().data(), self->getOriginalDirectory().length());
    return 1;
  } catch (std::exception &e) {
...
static int ofDirectory_getName(lua_State *L) {
  try {
    ...
    lua_pushlstring(L, self->getName(position).data(), self->getName(position).length());
    return 1;
  } catch (std::exception &e) {
...
static int ofDirectory_getPath(lua_State *L) {
  try {
    ofDirectory *self = *((ofDirectory **)dub_checksdata(L, 1, "ofDirectory"));
    unsigned int position = dub_checkint(L, 2);
    lua_pushlstring(L, self->getPath(position).data(), self->getPath(position).length());
    return 1;
  } catch (std::exception &e) {
...
static int ofDirectory_createDirectory(lua_State *L) {
  try {
    ...
    if (top__ >= 3) {
      size_t dirPath_sz_;
      const char *dirPath = dub_checklstring(L, 1, &dirPath_sz_);
      bool bRelativeToData = dub_checkboolean(L, 2);
      bool recursive = dub_checkboolean(L, 3);
      lua_pushboolean(L, ofDirectory::createDirectory(std::string(dirPath, dirPath_sz_), bRelativeToData, recursive));
      return 1;
    } else if (top__ >= 2) {
      size_t dirPath_sz_;
      const char *dirPath = dub_checklstring(L, 1, &dirPath_sz_);
      bool bRelativeToData = dub_checkboolean(L, 2);
      lua_pushboolean(L, ofDirectory::createDirectory(std::string(dirPath, dirPath_sz_), bRelativeToData));
      return 1;
    } else {
      size_t dirPath_sz_;
      const char *dirPath = dub_checklstring(L, 1, &dirPath_sz_);
      lua_pushboolean(L, ofDirectory::createDirectory(std::string(dirPath, dirPath_sz_)));
      return 1;
    }
  } catch (std::exception &e) {
...
static int ofDirectory_isDirectoryEmpty(lua_State *L) {
  try {
    ...
    if (top__ >= 2) {
      ...
      const char *dirPath = dub_checklstring(L, 1, &dirPath_sz_);
      bool bRelativeToData = dub_checkboolean(L, 2);
      lua_pushboolean(L, ofDirectory::isDirectoryEmpty(std::string(dirPath, dirPath_sz_), bRelativeToData));
      return 1;
    } else {
      size_t dirPath_sz_;
      const char *dirPath = dub_checklstring(L, 1, &dirPath_sz_);
      lua_pushboolean(L, ofDirectory::isDirectoryEmpty(std::string(dirPath, dirPath_sz_)));
      return 1;
    }
  } catch (std::exception &e) {
...
static int ofDirectory_doesDirectoryExist(lua_State *L) {
  try {
    ...
    if (top__ >= 2) {
      ...
      const char *dirPath = dub_checklstring(L, 1, &dirPath_sz_);
      bool bRelativeToData = dub_checkboolean(L, 2);
      lua_pushboolean(L, ofDirectory::doesDirectoryExist(std::string(dirPath, dirPath_sz_), bRelativeToData));
      return 1;
    } else {
      size_t dirPath_sz_;
      const char *dirPath = dub_checklstring(L, 1, &dirPath_sz_);
      lua_pushboolean(L, ofDirectory::doesDirectoryExist(std::string(dirPath, dirPath_sz_)));
      return 1;
    }
  } catch (std::exception &e) {
...
static int ofDirectory_removeDirectory(lua_State *L) {
  try {
    ...
    if (top__ >= 3) {
      ...
      bool bRelativeToData = dub_checkboolean(L, 3);
      lua_pushboolean(L, ofDirectory::removeDirectory(std::string(path, path_sz_), deleteIfNotEmpty, bRelativeToData));
      return 1;
    } else {
      size_t path_sz_;
      const char *path = dub_checklstring(L, 1, &path_sz_);
      bool deleteIfNotEmpty = dub_checkboolean(L, 2);
      lua_pushboolean(L, ofDirectory::removeDirectory(std::string(path, path_sz_), deleteIfNotEmpty));
      return 1;
    }
  } catch (std::exception &e) {
