...
static int ofPolyline_getClosestPoint(lua_State *L) {
  try {
    ...
    if (top__ >= 3) {
      ...
      unsigned int *nearestIndex = *((unsigned int **)dub_checksdata(L, 3, "unsigned int"));
      dub_pushudata(L, new ofVec3f(self->getClosestPoint(*target, nearestIndex)), "ofVec3f", true);
      return 1;
    } else {
      ofVec3f *target = *((ofVec3f **)dub_checksdata(L, 2, "ofVec3f"));
