...
static int ofMessage__set_(lua_State *L) {
  ...
  switch(key_h) {
    case 1: {
      ...
      self->message = **((string **)dub_checksdata_n(L, 3, "string"));
      return 0;
    }
  }
  if (lua_istable(L, 1)) {
...
static int ofMessage__get_(lua_State *L) {
  ...
  switch(key_h) {
    case 1: {
      if (DUB_ASSERT_KEY(key, "message")) break;
      dub_pushudata(L, &self->message, "string", false);
      return 1;
    }
  }
  return 0;
...
static int ofMessage_ofMessage(lua_State *L) {
  try {
    string *msg = *((string **)dub_checksdata(L, 1, "string"));
    ofMessage *retval__ = new ofMessage(*msg);
