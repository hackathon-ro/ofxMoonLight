...
static int ofHttpResponse__set_(lua_State *L) {
  ...
  switch(key_h) {
    ...
    case 1: {
      ...
      self->error = **((string **)dub_checksdata_n(L, 3, "string"));
      return 0;
    }
  }
  if (lua_istable(L, 1)) {
...
static int ofHttpResponse__get_(lua_State *L) {
  ...
  switch(key_h) {
    ...
    case 1: {
      if (DUB_ASSERT_KEY(key, "error")) break;
      dub_pushudata(L, &self->error, "string", false);
      return 1;
    }
  }
  return 0;
...
static int ofHttpResponse_ofHttpResponse(lua_State *L) {
  try {
    ...
    if (top__ >= 4) {
      ...
      string *error = *((string **)dub_checksdata(L, 4, "string"));
      ofHttpResponse *retval__ = new ofHttpResponse(*request, *data, status, *error);
      dub_pushudata(L, retval__, "ofHttpResponse", true);
      return 1;
    } else if (top__ >= 3) {
      ofHttpRequest *request = *((ofHttpRequest **)dub_checksdata(L, 1, "ofHttpRequest"));
      int status = dub_checkint(L, 2);
      string *error = *((string **)dub_checksdata(L, 3, "string"));
      ofHttpResponse *retval__ = new ofHttpResponse(*request, status, *error);
      dub_pushudata(L, retval__, "ofHttpResponse", true);
      return 1;
    } else {
