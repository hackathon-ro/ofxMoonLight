...
static int ofBuffer_getText(lua_State *L) {
  try {
    ofBuffer *self = *((ofBuffer **)dub_checksdata(L, 1, "ofBuffer"));
    lua_pushlstring(L, self->getText().data(), self->getText().length());
    return 1;
  } catch (std::exception &e) {
...
static int ofBuffer_getNextLine(lua_State *L) {
  try {
    ofBuffer *self = *((ofBuffer **)dub_checksdata(L, 1, "ofBuffer"));
    lua_pushlstring(L, self->getNextLine().data(), self->getNextLine().length());
    return 1;
  } catch (std::exception &e) {
...
static int ofBuffer_getFirstLine(lua_State *L) {
  try {
    ofBuffer *self = *((ofBuffer **)dub_checksdata(L, 1, "ofBuffer"));
    lua_pushlstring(L, self->getFirstLine().data(), self->getFirstLine().length());
    return 1;
  } catch (std::exception &e) {
