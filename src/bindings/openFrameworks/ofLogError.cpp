...
static int ofLogError_ofLogError(lua_State *L) {
  try {
    ...
    if (top__ >= 2) {
      ...
      ofLogError *retval__ = new ofLogError(std::string(module, module_sz_), std::string(message, message_sz_));
      dub_pushudata(L, retval__, "ofLogError", true);
      return 1;
    } else if (top__ >= 1) {
      size_t module_sz_;
      const char *module = dub_checklstring(L, 1, &module_sz_);
      ofLogError *retval__ = new ofLogError(std::string(module, module_sz_));
      dub_pushudata(L, retval__, "ofLogError", true);
      return 1;
    } else {
